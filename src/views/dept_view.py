...
from src.models.dept_model import insert_dept
...
class DeptView(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.ent_name = ttk.Entry(frame, width=33); self.ent_name.grid(row=0, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="Department Type:").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        self.cb_type = ttk.Combobox(frame, values=["Gov (လွှတ်တော်/အစိုးရအဖွဲ့/ကော်မတီ/တရားရေး)", "NonGov"], width=30); self.cb_type.grid(row=1, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="Department Level:").grid(row=2, column=0, sticky="w", padx=10, pady=5)
        self.cb_lvl = ttk.Combobox(frame, values=["ဝန်ကြီးဌာန (၁)", "ဦးစီးဌာန (၂)", "ဌာနခွဲ (၃)", "တိုင်းဒေသကြီး/ပြည်နယ် (၁)", "ခရိုင် (၂)", "မြို့နယ် (၃)"], width=30)
        self.cb_lvl.grid(row=2, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="Address:").grid(row=3, column=0, sticky="nw", padx=10, pady=5)
        self.txt_addr = tk.Text(frame, width=25, height=3); self.txt_addr.grid(row=3, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="Email (*@*):").grid(row=4, column=0, sticky="w", padx=10, pady=5)
        self.ent_email = ttk.Entry(frame, width=33); self.ent_email.grid(row=4, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="Phone Number:").grid(row=5, column=0, sticky="w", padx=10, pady=5)
        self.ent_phone = ttk.Entry(frame, width=33); self.ent_phone.grid(row=5, column=1, padx=10, pady=5)
        
        ttk.Label(frame, text="မှတ်ချက်:").grid(row=6, column=0, sticky="nw", padx=10, pady=5)
        self.txt_remark = tk.Text(frame, width=25, height=3); self.txt_remark.grid(row=6, column=1, padx=10, pady=5)
