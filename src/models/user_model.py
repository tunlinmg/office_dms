...
from src.models.role_model import get_permissions_for_role, get_role_names, fetch_department_permissions
...
def insert_user(username, password, full_name, email, department, role, is_active=1, current_user=None):
    ...
    if role not in valid_roles:
        logger.error("Invalid role '%s' for user '%s'", role, username)
        return False
    if not department.strip() and role != "admin":
        logger.warning("Department is required for non-admin user '%s'", username)
        return False
    conn = get_db_connection()
    if not conn:
        return False
    try:
        ...
        logger.exception("Could not insert user '%s'", username)
        return False
    finally:
        conn.close()
...
def update_user(user_id, username, full_name, email, department, role, is_active, password=None, current_user=None):
    ...
    try:
        ...
        logger.exception("Could not update user '%s'", username)
        return False
    finally:
        conn.close()
...
def update_user_role(user_id, role):
    valid_roles = get_role_names()
    if role not in valid_roles:
        logger.error("Invalid role '%s' for user ID %s", role, user_id)
        return False
    conn = get_db_connection()
    if not conn:
        return False
    try:
        ...
        logger.exception("Could not update role for user ID %s", user_id)
        return False
    finally:
        conn.close()
...
def delete_user(user_id, current_user_id, current_user=None):
    if user_id == current_user_id:
        logger.warning("User ID %s attempted to delete their own account", user_id)
        return False
    conn = get_db_connection()
    if not conn:
        return False
    try:
        ...
        if row and row[0] == "admin" and admin_count <= 1:
            logger.warning("Cannot delete last admin account (user ID %s)", user_id)
            return False
        ...
        logger.exception("Could not delete user ID %s", user_id)
        return False
    finally:
        conn.close()