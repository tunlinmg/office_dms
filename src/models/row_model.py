...
def fetch_rows(table_name, current_user=None):
    ...
    try:
        ...
        logger.exception("Could not fetch rows for table '%s'", table_name)
        return []
    finally:
        conn.close()
...
def update_row(table_name, pk_value, values):
    ...
    if len(values) != len(editable):
        logger.error("Column count mismatch for table '%s'", table_name)
        return False
    conn = get_db_connection()
    if not conn:
        return False
    try:
        ...
        logger.exception("Could not update row in table '%s'", table_name)
        return False
    finally:
        conn.close()
...
def delete_row(table_name, pk_value):
    ...
    try:
        ...
        logger.exception("Could not delete row from table '%s'", table_name)
        return False
    finally:
        conn.close()